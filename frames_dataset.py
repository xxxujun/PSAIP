...
def read_video(name, frame_shape):
    ...
    if os.path.isdir(name):
        ...
        if video.shape[-1] == 4:
            video = video[..., :3]
        video_array = img_as_float32(video) # 将图像转换为双精度浮点格式。
    else:
...
class FramesDataset(Dataset):
    ...
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
        ...
        self.videos = os.listdir(root_dir) # os.listdir()方法用于返回指定文件夹包含的文件或文件夹的名字的列表
...
class DatasetRepeater(Dataset):
    ...
    def __len__(self):
        return self.num_repeats * self.dataset.__len__() # =75*数据集长度（1）
